...
pub struct Peer {
    ...
    framed: Arc<crate::types::TcpMagic>,
    /// Handshake timeout.
        ...
        self.view_client_addr
            ...
            .then(move |res, act, _ctx| match res {
                Ok(NetworkViewClientResponses::ChainInfo {
                    ...
                    act.send_message(&handshake, _ctx);
                    actix::fut::ready(())
                }
                Err(err) => {
                    error!(target: "network", "Failed sending GetChain to client: {}", err);
            ...
            .then(move |res, act, _ctx| {
                ...
                match res {
                    ...
                    Ok(NetworkViewClientResponses::Block(block)) => {
                        ...
                        act.send_message(&PeerMessage::Block(*block), _ctx)
                    }
                    Ok(NetworkViewClientResponses::BlockHeaders(headers)) => {
                        act.send_message(&PeerMessage::BlockHeaders(headers), _ctx)
                    }
                    Err(err) => {
                        error!(
                            target: "network",
                ...
                self.peer_manager_addr.send(PeersRequest {}).into_actor(self).then(|res, act, _ctx| {
                    if let Ok(peers) = res {
                        if !peers.peers.is_empty() {
                            ...
                            act.send_message(&PeerMessage::PeersResponse(peers.peers), _ctx);
                        }
                    }
                    actix::fut::ready(())
                }).spawn(ctx);
